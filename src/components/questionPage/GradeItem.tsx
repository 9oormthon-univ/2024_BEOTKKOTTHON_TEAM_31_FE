...
const GradeItem = ({ category, quiz, isSimiler }: { category: string; quiz: GradeItemType; isSimiler: boolean }) => {
  const navigate = useNavigate();
  return (
    <div
      onClick={() => {
        isSimiler
          ? navigate(`/similar/grade/${category}/${quiz.quizId}`, { state: { category: quiz.category } })
          : navigate(`/explain/${quiz.quizId}`, { state: { category: quiz.category } });
      }}
